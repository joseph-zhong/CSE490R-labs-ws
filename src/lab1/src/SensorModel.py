...
class SensorModel:
  def __init__(self, map_msg, particles, weights, state_lock=None):
    ...
    self.range_method = range_libc.PyCDDTCast(oMap, max_range_px,
      THETA_DISCRETIZATION)  # The range method that will be used for ray casting
  ...
  def lidar_cb(self, msg):
    ...
    # Keep efficiency in mind, including by caching certain things that won't change across future iterations of this
    #  callback

    # YOUR CODE HERE
    # Tam - Downsampled angles shouldn't change.
    if self.downsampled_angles is None:
      self.downsampled_angles = np.arange(msg.angle_min, msg.angle_max, msg.angle_increment * self.LASER_RAY_STEP,
        dtype=np.float32)
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    def interpolated_pdf(observed, expected):
      ...
      p_hit = (1 / np.sqrt(2 * np.pi * sigma_squared)) * np.exp(
        -(np.power(observed - expected, 2) / (2 * sigma_squared)))

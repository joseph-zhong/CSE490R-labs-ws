...
class MPPIController(object):
  def __init__(self):
    ...
    # print "Loading DNN model_name:", model_name
    # print "Loading DNN model:\n", self.model
    # print "Loading DNN datatype:", self.dtype
    ...
    self.ctrl_pub = rospy.Publisher(rospy.get_param("~ctrl_topic",
      "/vesc/high_level/ackermann_cmd_mux/input/nav_0"), AckermannDriveStamped, queue_size=2)
    self.path_pub = rospy.Publisher("/mppi/paths", Path, queue_size=self.num_viz_paths)

    # Use the 'static_map' service (launched by MapServer.launch) to get the map
    print "Setting up map..."
    map_service_name = rospy.get_param("~static_map", "static_map")
    print "Getting map service name: ", map_service_name
    ...
    print "Successfully got map: map_info:", self.map_info
    ...
    # PATH_NAME = '/home/nvidia/catkin_ws/src/lab4/paths/full_dubins_path_10_300.npy'
    PATH_NAME = '/home/josephz/Dropbox/UW/CSE490R/labs/src/lab4/paths/full_dubins_path_10_300.npy'
    assert os.path.isfile(PATH_NAME)
    self.path = np.load(PATH_NAME)
    self.path = np.load(PATH_NAME)
    ...
    # LOAD MAP DATA!!!
    MAP_DATA_FNAME = "/home/josephz/Dropbox/UW/CSE490R/labs/src/lab4/maps/sieg_floor3_radius5.pgm"
    print "LOADING MAP DATA FROM ", MAP_DATA_FNAME
    import matplotlib.pyplot as plt
    array255_test = np.sum(plt.imread(MAP_DATA_FNAME)[::-1, :, :], axis=-1)
    test_permissable = np.zeros_like(array255_test, dtype=bool)
    test_permissable[array255_test == 0] = 1
    test_permissable = test_permissable == 1
    test_permissable = test_permissable.astype(int)
    print "OBSTRUCTED MAP", array255_test.shape
    plt.imshow(test_permissable)
    plt.show()
    import cv2
    # array255_test = cv2.cvtColor(array255_test, cv2.COLOR_RGB2GRAY)

    # test_permissible_region = np.zeros_like(array255_test, dtype=bool)
    # test_permissible_region[array255_test == 0] = 1  # Numpy array of dimension (map_msg.info.height, map_msg.info.width),
    # # With values 0: not permissible, 1: permissible
    # test_permissible_region = np.negative(test_permissible_region)  # 0 is permissible, 1 is not
    # test_permissible_region = test_permissible_region.astype(int)
    # test_permissible_region = torch.cuda.IntTensor(test_permissible_region)
    ...
    import pdb
    pdb.set_trace()
    print "array255_test.shape == array_255.shape", array255_test.shape == array_255.shape
    print "np.sum(array255_test == array_255)", np.sum(array255_test == array_255)
    print "np.sum(array_255)", np.sum(array_255)
    ...
    print "Setting up goal waypoints to visualize..."

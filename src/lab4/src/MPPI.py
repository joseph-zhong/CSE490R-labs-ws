...
class MPPIController(object):
  def __init__(self):
    ...
    assert os.path.isfile('paths/full_dubins_path.npy')
    self.path = np.load('paths/full_dubins_path.npy')

    # TODO: Make next place in path the first goal
    self.place_in_path = 0  # Assumes robot is placed at path[0]
    self.goal = self.path[1] # Lets keep track of the goal pose (world frame) over time
    self.lasttime = None
  ...
  def is_far_from_goal(self, curr_pose):
    ...
    print "exiting is_far_to_goal, diff:", diff, "angle_between:", angle_between, "is close?", is_close
    return is_close
  ...
  def mppi(self, curr_pose, init_input):
    ...
    if self.is_far_from_goal(curr_pose):
      ...
      self.place_in_path += 1
      x, y, theta = self.path[self.place_in_path]
      self.goal = torch.cuda.FloatTensor([x, y, Utils.quaternion_to_angle(theta)])
